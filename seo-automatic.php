<?php
/*
Plugin Name: SEO Tool Add-on - URL Review Lite (activate separately)
...
function myErrorHandler($errno, $errstr, $errfile, $errline) {
	switch ($errno) {
	default:
		$all_errors = "[$errno] $errstr - $errfile - $errline<br />\n";
		if ($all_errors) { echo ' '; }
		if (strstr($all_errors, 'is_a()')) { $all_errors = ''; }
		if (strstr($all_errors, 'seoautomatic')) {
			echo '<pre>'; print_r($all_errors); echo '</pre>';
		} else {
			echo '';
		}
	}
...
<h3><span><img src="http://www.seoautomatic.com/favicon.ico" alt="SEO Automatic" /> SEO Tool Add-on - URL Review Lite (activate separately)</span></h3>
<div class="inside">
<?php
